...
struct UnstackerMetadata {
  static StatusOr<UnstackerMetadata> Create(HloModule* module) {
    UnstackerMetadata context;
    ...
    for (const auto& [instr, while_loop_config] : loops) {
      context.unrollable_loop_bodies[instr->while_body()] = while_loop_config;
    }
    return context;
  }
...
struct UnstackerTransformer {
  UnstackerMetadata context;
  static StatusOr<UnstackerTransformer> Create(const UnstackerMetadata& c) {
    UnstackerTransformer transformer;
    transformer.context = std::move(c);
    return transformer;
  }
  ...
  bool HandleInstruction(const HloInstruction* instr, int64_t changed_idx) {
    ...
    auto custom_pattern = context.custom_handler.first;
    auto custom_handler = context.custom_handler.second;

    const HloInstruction* stacked_user =
        custom_pattern(context, instr, changed_idx);
...
// index.
bool UnstackHelper(const UnstackerMetadata& context,
                   HloInstruction* while_instr, int64_t index) {
  UnstackerTransformer unstacker =
      UnstackerTransformer::Create(context).value();
...
const HloInstruction* IsNestedDynamicSlicingFusion(
    const UnstackerMetadata& context, const HloInstruction* instr,
    int64_t stacked_operand_idx) {
  if (!Match(instr, match::Fusion())) {
    return nullptr;
  }

  if (!context.unrollable_loop_bodies.contains(instr->parent())) {
  ...
  WhileLoopConfig while_instr_config =
      context.unrollable_loop_bodies.at(instr->parent());
...
// unstacking context and populates the unstacking custom handler(s). Moreover,
...
absl::StatusOr<bool> HloUnstacker::Run(
  ...
  TF_ASSIGN_OR_RETURN(auto context, UnstackerMetadata::Create(module));

  context.custom_handler = std::make_pair(IsNestedDynamicSlicingFusion,
  ...
  for (HloInstruction* instr :
    ...
    for (int64_t i = 0; i < instr->shape().tuple_shapes_size(); ++i) {
      ...
      if (UnstackHelper(context, instr, i)) {
