...
Shape GetShapeFromTensorType(mlir::Value value) {
  ...
  CHECK(op);
  CHECK(isa<mlir::TensorType>(value.getType()));
  Shape shape;
...
static int64_t GetMemRefSizeInBytes(mlir::MemRefType type) {
  ...
  if (type.getElementType().isInteger(/*width=*/1)) {
    return type.getNumElements();
                 ...
                 dyn_cast<mlir::ComplexType>(type.getElementType())) {
...
Shape GetShape(mlir::Value value) {
  Shape shape;
  if (isa<mlir::MemRefType>(value.getType())) {
    shape = TypeToShape(value.getType());
  } else if (isa<mlir::TensorType>(value.getType())) {
    shape = GetShapeFromTensorType(value);
  } else if (isa<mlir::TupleType>(value.getType())) {
    shape = TypeToShape(value.getType());
  } else {
