...
LogicalResult ConvertTFLRelu0To1Op::matchAndRewrite(
  ...
  ShapedType input_type = cast<ShapedType>(tfl_relu0to1_op.getX().getType());
  ShapedType output_type =
      cast<ShapedType>(tfl_relu0to1_op.getResult().getType());
...
static LogicalResult prepareMatchAndRewriteComparison(
  ...
  bool input_x_is_qtype =
      isa<mlir::quant::UniformQuantizedType>(input_x_type.getElementType());
  bool input_y_is_qtype =
      isa<mlir::quant::UniformQuantizedType>(input_y_type.getElementType());
  bool output_is_qtype =
      isa<mlir::quant::UniformQuantizedType>(output_type.getElementType());
...
static LogicalResult matchAndRewriteAddSub(Operation* op,
  ...
  ShapedType input_lhs_type =
      dyn_cast<ShapedType>(tfl_add_op.getLhs().getType());
  ShapedType input_rhs_type =
      dyn_cast<ShapedType>(tfl_add_op.getRhs().getType());
  ShapedType output_type =
      dyn_cast<ShapedType>(tfl_add_op.getResult().getType());
  // Not a ranked tensor output
...
LogicalResult ConvertTFLSignOp::matchAndRewrite(
  ...
  RankedTensorType output_type =
      cast<RankedTensorType>(tfl_sign_op.getResult().getType());
...
LogicalResult ConvertTFLAveragePool2DOp::matchAndRewrite(
  ...
  TypeAttr acc_attr = isa<FloatType>(average_etype)
                          ? mlir::TypeAttr::get(rewriter.getF32Type())
                          : mlir::TypeAttr::get(rewriter.getIntegerType(32));

  Value result;
  if (isa<quant::UniformQuantizedType>(average_etype)) {
...
LogicalResult ConvertTFLConv2DOp::matchAndRewrite(
  ...
  if (unquantized_bias) {
    ...
    if (auto qtype = dyn_cast<mlir::quant::QuantizedType>(new_bias_ety)) {
...
LogicalResult ConvertTFLConv3DOp::matchAndRewrite(
  ...
  if (!dyn_cast<RankedTensorType>(unquantized_bias.getType())) {
    ...
    unquantized_bias = CreateOpAndInfer<tosa::ConstOp>(
        rewriter, op->getLoc(), bias_type, cast<ElementsAttr>(bias_attr));
  }
  ...
  Type bias_ety = cast<ShapedType>(unquantized_bias.getType()).getElementType();
...
LogicalResult ConvertTFLTransposeConvOp::matchAndRewrite(
  ...
  Type bias_ety = cast<ShapedType>(zero_bias->getType()).getElementType();
...
LogicalResult ConvertTFLBatchMatMulOp::matchAndRewrite(
  ...
  auto result_ty = cast<ShapedType>(tfl_mm_op.getType());
  ...
  bool lhs_is_qtype = isa<mlir::quant::QuantizedType>(lhs_ty.getElementType());
  bool rhs_is_qtype = isa<mlir::quant::QuantizedType>(rhs_ty.getElementType());
  bool result_is_qtype =
      isa<mlir::quant::QuantizedType>(result_ty.getElementType());
  ...
  if (batch_dims.size() != 1) {
    ...
    lhs_ty = cast<RankedTensorType>(lhs.getType());
    rhs_ty = cast<RankedTensorType>(rhs.getType());
  }
  ...
  if (result_is_qtype) {
    auto lhs_qty_width =
        cast<mlir::quant::QuantizedType>(lhs_ty.getElementType())
            .getStorageTypeIntegralWidth();
    auto rhs_qty_width =
        cast<mlir::quant::QuantizedType>(rhs_ty.getElementType())
            .getStorageTypeIntegralWidth();
  ...
  auto matmul_ty = cast<RankedTensorType>(matmul.getType());
  if (batch_dims.size() != 1) {
...
LogicalResult ConvertTFLFullyConnectedOp::matchAndRewrite(
  ...
  ShapedType fc_type = cast<ShapedType>(fc_output.getType());
  if (output_type.hasRank()) {
...
LogicalResult ConvertTFLRankOp::matchAndRewrite(
  ...
  auto rank_const = CreateOpAndInfer<tosa::ConstOp>(
      rewriter, op->getLoc(), rank_type, cast<ElementsAttr>(rank_attr));
...
LogicalResult ConvertTFLShapeOp::matchAndRewrite(
  ...
  auto shape_const = CreateOpAndInfer<tosa::ConstOp>(
      rewriter, op->getLoc(), shape_type, cast<ElementsAttr>(shape_attr));
...
LogicalResult ConvertTFLFillOp::matchAndRewrite(
  ...
  if (isa<FloatType>(value_elem.getShapedType().getElementType())) {
  ...
  auto fill_const_op = CreateOpAndInfer<tosa::ConstOp>(
      rewriter, op->getLoc(), fill_type, cast<ElementsAttr>(fill_attr));
...
LogicalResult ConvertTFLCosOp::matchAndRewrite(
  ...
  bool input_is_fp = isa<mlir::FloatType>(input_ty.getElementType());
  bool output_is_fp = isa<mlir::FloatType>(output_ty.getElementType());
...
static LogicalResult LegalizeQuantizedPrelu(Operation* op,
  ...
  ShapedType alpha_type = cast<ShapedType>(alpha.getType());
  UniformQuantizedType alpha_qtype =
      cast<UniformQuantizedType>(alpha_type.getElementType());
...
static LogicalResult LegalizeQuantizedLeakyRelu(Operation* op,
                                                ...
                                                ShapedType output_type) {
  ShapedType input_type = cast<ShapedType>(input.getType());
...
LogicalResult ConvertTFLCustomOp::matchAndRewrite(
  ...
  rewriter.replaceOpWithNewOp<tosa::CustomOp>(
      ...
      cast<mlir::TFL::ConstBytesAttr>(tfl_custom_op.getCustomOption())
          .getValue()
          .str(),
      op->getOperands());
...
LogicalResult ConvertTFLConstOp::matchAndRewrite(
  ...
  if (isa<quant::QuantizedType>(output_type.getElementType())) {
  ...
  // unranked shapes.
  if (!output_type.hasRank()) {
    output_type = cast<ShapedType>(elements.getType()).clone(element_type);
  }
...
LogicalResult ConvertTFLQConstOp::matchAndRewrite(
  ...
  // unranked shapes.
  if (!output_type.hasRank()) {
    output_type = cast<ShapedType>(elements.getType())
...
LogicalResult ConvertConstantOp::matchAndRewrite(
  ...
  if (e_type.isInteger(64)) {
    ...
    attr = cast<DenseIntOrFPElementsAttr>(attr).mapValues(
...
LogicalResult ConvertTFLSparseToDenseOp::matchAndRewrite(
  ...
  auto indices_ty = cast<ShapedType>(indices.getType());
  auto indices_ety = indices_ty.getElementType();
  auto values_ty = cast<ShapedType>(values.getType());
  auto result_ty =
      cast<ShapedType>(tfl_sparse_to_dense_op.getResult().getType());
...
LogicalResult ConvertTFLRealOp::matchAndRewrite(
  ...
  if (!isa<ComplexType>(input_ety)) {
    CreateReplaceOpAndInfer<tosa::IdentityOp>(rewriter, op, input_ty, input);
    return success();
  }

  if (!cast<ComplexType>(input_ety).getElementType().isF32()) {
    return rewriter.notifyMatchFailure(
